...
addpath('functions','mfiles');



load priors
load results
fsize=50; %number of MCMC iterations
HORZ=40;   % horizon
scale=1;   % shock size=scale standard deviations
reps=50;   %number of simulations to estimate IRF
pos=1;  %shock is inserted in equation number pos
pattern=[-1 -1 1 1; %policy shock% 0=no restriction, 1 means >0 and -1 <0
...
A01c=chol(s1)';
A02c=chol(s2)';
 [~,Pmat1]=var_sign_chol_restictions(A01c,pattern(:));
    Rmat1=A01c*Pmat1;
 [~,Pmat2]=var_sign_chol_restictions(A02c,pattern(:));
    Rmat2=A02c*Pmat2;
A01=Rmat1';
A02=Rmat2';
